# Hat3D — mode détaillé : donner du volume
...
rangé en `hat3d/<famille>/<slug>/` :

```js
import { fabriqueVolume } from "../../../.claude/skills/hat3d/lib/volume.mjs";
...
`export * from "../../.claude/skills/hat3d/lib/volume.mjs";` dans
`hat3d/_lib/volume.mjs` — donne un import court (`../../_lib/volume.mjs`) et
...
MeshPart (Blender, EditableMesh) — hors périmètre Hat3D, qui garde en
...
lourdes y arrive même en compact, car c'est le module `Hat3DAnim` embarqué
...
Le panneau de la préview affiche maintenant un compte de **triangles estimés**
à côté du nombre de parts (tessellation du viewer, un ordre de grandeur — pas
la tessellation exacte de Roblox). Un `tour` à 24 segments et un `Block`
comptent tous deux pour « 1 part » mais coûtent très différemment en rendu :
c'est ce chiffre-là qu'il faut regarder avant d'instancier un modèle en masse,
pas le nombre de parts seul.
