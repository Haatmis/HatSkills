# Hat3D — animations (`model.json` → préview jouable → player Lua)

Même principe que le reste d'Hat3D : les animations sont décrites dans
`model.json`, la préview HTML les **joue** (lecture, boucle, scrubber), et
`build.lua` embarque un ModuleScript `Hat3DAnim` qui rejoue **exactement
...
`build.lua` crée le ModuleScript `Hat3DAnim` dans le modèle :

```lua
local anim = require(workspace.CoffreFort.Hat3DAnim)
...
- **`windows`** = fenêtres `[tOn, tOff]` par animation. `Hat3DAnim` allume et
