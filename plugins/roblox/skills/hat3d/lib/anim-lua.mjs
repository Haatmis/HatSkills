/* Hat3DAnim — generation du ModuleScript player d'animations.
 *
 * Partage entre les deux pipelines, pour qu'il n'existe qu'UN player :
 *   - hat3d/scripts/build.mjs                  (modele en Parts)
 *   - hat3d-blender/scripts/assemble.mjs       (modele en MeshParts)
 *
...
export function sourceAnimLua(model) {
  ...
  const animSrc = `-- Hat3DAnim — player d'animations généré depuis model.json.
...
--   local anim = require(<modele>.Hat3DAnim)
...
local function findAnim(name)
	...
	error("[Hat3DAnim] animation inconnue : " .. tostring(name))
end
