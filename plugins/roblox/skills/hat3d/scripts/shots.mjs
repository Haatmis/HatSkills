#!/usr/bin/env node
// shots.mjs — captures de contrôle d'un modèle Hat3D, ou de tout un lot.
//
//   node .claude/skills/hat3d/scripts/shots.mjs hat3d/creature/fenrir
//   node .claude/skills/hat3d/scripts/shots.mjs hat3d/vegetation
//   node .claude/skills/hat3d/scripts/shots.mjs hat3d/boss --vue face
//
...
import { existsSync, readdirSync, statSync, rmSync, copyFileSync } from "node:fs";
...
for (const d of dossiers) {
  ...
  // La capture d'avant survit un cran (shot-3q.prev.png) avant d'être
  // écrasée : diff.mjs compare les deux pour repérer ce qu'une itération a
  // vraiment changé, sans que l'appelant ait à penser à la sauvegarder.
  if (existsSync(sortie)) { try { copyFileSync(sortie, sortie.replace(/\.png$/, ".prev.png")); } catch { /* best effort */ } }
  // On EFFACE la cible ensuite : Chrome laisse parfois le PNG précédent en
  ...
  try {
    execFileSync(CHROME, [
      ...
      "--user-data-dir=" + join(process.env.TEMP || "/tmp", "hat3d-shots"),
