#!/usr/bin/env node
/**
 * Hat3D build — transforme un model.json en :
...
const luaBody = [];
luaBody.push(`--------------------------------------------------------------------
-- ${model.name} — généré par Hat3D depuis model.json
...
function luaEmitter(e) {
  ...
  const L = [
    ...
    `\tassert(host, "[Hat3D] emitter ${e.name} : part ${e.parent} introuvable")`,
  ];
  if (e.offset) {
    L.push(
...
// (hat3d-blender/scripts/assemble.mjs) : un seul player, deux pipelines.
{
...
// `builtParts` recompte les BaseParts réellement présentes sous `model` une
// fois construit, plutôt que de ré-imprimer `model.parts.length` en dur : un
// bug d'encodage compact (table tronquée) construirait alors moins de parts
// que prévu sans jamais lever d'erreur Luau — le récap mentait par défaut.
luaBody.push(`
model.Parent = CONFIG.PARENT

local builtParts = 0
for _, inst in ipairs(model:GetDescendants()) do
	if inst:IsA("BasePart") and inst.Name ~= "Root" then
		builtParts += 1
	end
end
local EXPECTED_PARTS = ${model.parts.length}
if builtParts == EXPECTED_PARTS then
	print(("[Hat3D] %s construit : %d parts, %s studs${fxCount ? ", " + fxCount + " emetteur(s) de particules" : ""}${animCount ? ", " + animCount + " animation(s) — require(model.Hat3DAnim).play(\\\"" + model.animations[0].name + "\\\")" : ""}"):format(
		MODEL_NAME, builtParts, "${dims.map(d => fmt(d)).join(" x ")}"))
else
	warn(("[Hat3D] %s : %d/%d parts construites — le build s'est arrêté en cours de route (voir l'erreur au-dessus dans la Sortie)."):format(
		MODEL_NAME, builtParts, EXPECTED_PARTS))
end`);
